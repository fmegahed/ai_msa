...
Please complete the upper triangular part of the following 13x13 adjacency matrix (based on the approach of Ford and Hegarty, 1984, Decision makers' beliefs about the causes and effects of structure: An exploratory study. Academy of Management Journal, 27[2], 271-291) to denote the causal relationships among these constructs:
...
Your analysis will provide insights into how these factors interact and influence user perceptions of LLM outputs. Ensure to indicate the type and direction of each relationship, or mark it as non-existent where appropriate. We supplement the approach in Ford and Hagarty (1984) with detailed instructions that will be shared in the user prompt. 
"""

user_prompt = f"""
To complete the adjacency matrix for the 13 dimensions of information quality with respect to outputs from large language models (LLMs), please adhere to the following definitive steps:
...
1. **Evaluation Criteria**:
   ...
   - It is unlikely that all the relationships will be positive, as some constructs may have negative impacts on others.
   - Given the structured nature of the adjacency matrix, we expect a mix of no relationships (O), row (A+ or A-) and column (B+ or B-) causal relationships to be present. So think critically about the interplay between these constructs.
...
# ------------------------------------------------------------------------------
# Function to Extract the Response Table:
# ------------------------------
# The response table is extracted from the chat completion response.

header_string = "A/B,Accuracy,Trustworthness,Objectivity,Appropriateness,Relevance,Completeness,Conciseness,Consistency,Underestandability,Interpretability,Flexibility,Satisfaction,Use Intention"

last_row = "Use Intention,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN"

def extract_table_from_text(text, start_marker, end_marker):
    # Find the start and end indices of the specified markers
    start_index = text.find(start_marker)
    end_index = text.find(end_marker)
    
    # Adjust the end index to include the length of the end_marker
    if end_index != -1:
        end_index += len(end_marker)
    
    # Extract the table text between the start and end indices
    if start_index != -1 and end_index != -1 and end_index > start_index:
        table_text = text[start_index:end_index]
        return table_text
    else:
        return None  # Return None if the table is not found or markers are incorrect
...
for model, replicate in combinations:
  try:
    ...
    # meta data passed to the response
    ...
    # adding the meta data to the response
    response += f"""\n\nThe response above was generated starting at {generation_time}, using the {model} model. The completion was generated with a temperature of 0.0 and a maximum token length of 3000. The code utilized the Langchain module to interact with the model and generate the completions. The Langchain version used was {langchain_version}, with two submodules: Langchain-OpenAI (v. {langchain_openai_version}) and Langchain-Anthropic version (v. {langchain_anthropic_version})."""
    
    # Sleeping for 60 seconds to avoid rate limits
    print(f"Completed {key}. Sleeping for 60 seconds.")
    time.sleep(60)
    ...
    table_text = extract_table_from_text(response, header_string, last_row)
    if table_text:
        data = StringIO(table_text)
        df = pd.read_csv(data, index_col=0, header=0, skipinitialspace=True, engine='python')
        df.index = df.columns
    else:
        print("Table not found or markers are incorrect")
    
    # adding the df to the dict
    dict_of_dfs[key] = df
...
# ------------------------------------------------------------------------------
# Dictionaries of DFs for Both Models:
# ---------------------------------
...
# Extracting the headers from the first DataFrame
headers = gpt_dfs[gpt_keys[0]].columns

# Extracting the numeric values from the DataFrames
...
gpts_mode_labels = pd.DataFrame(gpts_mode_labels, columns=headers, index=headers)
gpts_perc_agreement = pd.DataFrame(gpts_perc_agreement, columns=headers, index=headers)

claudes_mode_labels, claudes_perc_agreement = calculate_percentage_agreement(combined_claudes)
claudes_mode_labels = pd.DataFrame(claudes_mode_labels, columns=headers, index=headers)
claudes_perc_agreement = pd.DataFrame(claudes_perc_agreement, columns=headers, index=headers)
...
def create_heatmap(df, label_df, model_name, model_print_name):
    ...
    annot_labels_df = pd.DataFrame(index=label_df.index, columns=label_df.columns)
    
    # Create the annotation labels for the heatmap
    for (idx, label_row), (_, value_row) in zip(label_df.iterrows(), df.iterrows()):
        annot_labels_df.loc[idx] = ["{0} ({1:.0f}%)".format(label, value) if pd.notna(value) else np.nan for label, value in zip(label_row, value_row)]
    ...
    heatmap = sns.heatmap(
      df, annot=annot_labels_df, fmt="", cmap=cmap, linewidths=0.15,
      linecolor="white",
      annot_kws={"fontsize": 8, "color": "black"},
      vmin=0, vmax=100, mask=df.isnull(),
      cbar=False,
