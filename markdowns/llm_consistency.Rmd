...
### Tabulating Simple Percent Agreement

```{r binary_percent_agreement_viz, results='asis'}
# Summarize the distribution of percent agreement by chat model
summary_table = 
  binary_analysis_df |>
  dplyr::group_by(chat_model) |>
  dplyr::summarize(
    count = dplyr::n(),
    mean_agreement = mean(percent_agreement, na.rm = TRUE),
    sd_agreement = sd(percent_agreement, na.rm = TRUE),
    q1 = quantile(percent_agreement, 0.25, na.rm = TRUE),
    q3 = quantile(percent_agreement, 0.75, na.rm = TRUE),
    min_agreement = min(percent_agreement, na.rm = TRUE),
    max_agreement = max(percent_agreement, na.rm = TRUE)
  ) |>
  dplyr::arrange(desc(mean_agreement))

# print the table nicely with gt
gt::gt(summary_table) |>
  gt::fmt_number(columns = c('mean_agreement', 'sd_agreement', 'q1', 'q3', 'min_agreement', 'max_agreement'), decimals = 2) |>
  gt::tab_header(title = "Summary of Percent Agreement by Chat Model") |>
  gt::fmt_number(columns = c('count'), use_seps = TRUE) |>
  gt::fmt_number(columns = c('mean_agreement', 'sd_agreement', 'q1', 'q3', 'min_agreement', 'max_agreement'), use_seps = TRUE) |>
  gt::fmt_missing(columns = c('mean_agreement', 'sd_agreement', 'q1', 'q3', 'min_agreement', 'max_agreement'), missing_text = "NA")
```
