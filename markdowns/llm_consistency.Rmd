...
# Sample Size Calculations for the Different Experiments {#sample_size}
...
## Minimum Sample Size for the Binary Classification Experiment {#min_sample_size_bin}
...
## Minimum Sample Size for the Multiclass Classification Experiment {#min_sample_size_multi}
...
### Extracting the LLM Labels and Creating a Dataframe for Analysis {#extract_llm_labels}
...
## Validity of the Binary Classification Labels (Comparison with "StockNewsAPI" Labels) {#validity_bin_api}
...
In the code chunk below, we use Python and the `yfinance` package to extract the stock prices for the tickers associated with the binary classification labels. We then save the raw stock prices to a CSV file for further analysis.

```{python binary_ground_truth_stock_performance, cache = TRUE, message=FALSE, warning=FALSE, results='hide'}
import pandas as pd
import datetime as dt
...
tickers_df = ( 
  pd.read_csv("../results/binary_analysis_df.csv")
  .assign(date = lambda x: pd.to_datetime(x["date"]))
  .filter(["date", "tickers"])
  )

# extract the unique tickers
tickers_list = tickers_df["tickers"].unique().tolist()
tickers_list.append("^GSPC")

# min and max dates
min_date = ( tickers_df["date"].min() - dt.timedelta(days=5) ).strftime("%Y-%m-%d")
max_date = ( tickers_df["date"].max() + dt.timedelta(days=5) ).strftime("%Y-%m-%d")

# download the stock prices
stock_prices = yf.download(tickers_list, start=min_date, end=max_date)

# convert to long format
stock_prices_long = (
  stock_prices['Adj Close'].reset_index()
  .melt(id_vars=["Date"], var_name="Ticker", value_name="Adj Close")
  .rename(columns={"Date": "date", "Ticker": "ticker", "Adj Close": "adjusted"})
  # convert date to a date string
  .assign(date = lambda x: x["date"].dt.strftime("%Y-%m-%d"))
  )

# save the stock prices to a CSV file
stock_prices_long.to_csv("../results/stock_prices_raw.csv", index=False)
```
...
news_effect_table = 
  table(binary_gold_df$leading_news_effect, useNA = "ifany") / 7
...
cat(
  "The table below shows the average distribution of the leading news effect, normalized by the number of LLM models (7):",
...
### Computing the Agreement with the Ground Truth Stock Performance

In the code chunk below, we calculate the agreement between the binary classification labels and the leading news effect. We calculate the agreement using two different methods:  
  - one that penalizes `NA`, and  
  - one that ignores `NA`.

```{r binary_agreement_with_ground_truth, results='asis', message=FALSE}
# calculate the agreement with the ground truth
binary_gold_validity_df = 
  binary_gold_df |>
  dplyr::rowwise() |>
  dplyr::mutate(
    agreement_na_penalty = calculate_agreement(
      reps = dplyr::c_across(dplyr::starts_with("rep_")),
      ground_truth = leading_news_effect
    ),
    agreement_na_as_na = calculate_agreement(
      reps = dplyr::c_across(dplyr::starts_with("rep_")),
      ground_truth = leading_news_effect,
...
# save the validity dataframe as RDS and CSV files
readr::write_rds(binary_gold_validity_df, "../results/binary_gold_validity_df.rds")
readr::write_csv(binary_gold_validity_df, "../results/binary_gold_validity_df.csv")
...
```{r binary_gold_validity_table, results='asis'}
binary_gold_validity_df |> 
  dplyr::group_by(chat_model) |>
  dplyr::summarize(
    `Mean Agreement (NA Penalty)` = mean(agreement_na_penalty) |> round(digits = 3),
...
# calculate the maximum agreement values for styling
max_agreement_na_penalty = max(gold_validity_table$`Mean Agreement (NA Penalty)`) 
max_agreement_na_dropped = max(gold_validity_table$`Mean Agreement (NA Dropped)`) 
...
#### By Model and News Effect {-}

```{r binary_validity_table_by_news, results='asis'}
binary_gold_validity_df |> 
  dplyr::group_by(chat_model, leading_news_effect) |>
  dplyr::summarize(
    `Mean Agreement (NA Penalty)` = mean(agreement_na_penalty) |> round(digits = 3),
    `Mean Agreement (NA Dropped)` = mean(agreement_na_as_na, na.rm = TRUE) |> round(digits = 3)
  ) |> 
  dplyr::ungroup() |> 
  dplyr::mutate(
    `API Costs / 1M Input Tokens` = c(3,3,3,  0.25,0.25,0.25, 2.5,2.5,2.5, 2.5,2.5,2.5, 0.15,0.15,0.15, 2,2,2, 0.2,0.2,0.2),
    `API Costs / 1M Output Tokens`= c(15,15,15,  1.25,1.25,1.25,  10,10,10,  10,10,10,  0.6,0.6,0.6,  6,6,6,  0.6,0.6,0.6),
    input_colors = dplyr::case_when(
      `API Costs / 1M Input Tokens` <= 0.25 ~ "#fcae91",
      `API Costs / 1M Input Tokens` <= 1 ~ "#fcae91",
      `API Costs / 1M Input Tokens` <= 2 ~ "#de2d26",
  ...
  ) -> validity_table_by_news

# calculate the maximum agreement values for styling
max_agreement_na_penalty = max(validity_table_by_news$`Mean Agreement (NA Penalty)`)
max_agreement_na_dropped = max(validity_table_by_news$`Mean Agreement (NA Dropped)`)

# render the table
validity_table_by_news |> 
  dplyr::select(chat_model, leading_news_effect, `API Costs / 1M Input Tokens`, `API Costs / 1M Output Tokens`, dplyr::everything()) |>
  dplyr::arrange(leading_news_effect, dplyr::desc(`Mean Agreement (NA Penalty)`)) |>
  dplyr::filter(
    leading_news_effect %in% c("Negative", "Positive") 
  ) |>
  DT::datatable(
    rownames = FALSE, 
    extensions = c("FixedColumns"),
...
The validity metrics for the binary classification experiment are visualized in the four plots below. The first two plots show the agreement with the `ground_truth` by LLM model, with the first plot penalizing `NAs` and the second plot ignoring `NAs`. The third and fourth plots show the agreement with the `ground_truth` by LLM model and sentiment, with the first plot penalizing `NAs` and the second plot ignoring `NAs`.

#### Overall NA Penalized Agreement {-}

```{r gold_binary_validity_plot, results='hold', fig.width=6.5, fig.height=8, fig.align='center', out.width="100%", class.source = 'fold-hide', warning=FALSE, message=FALSE}
...
binary_gold_validity_df |>
  ...
  ggplot2::geom_text(ggplot2::aes(label = sprintf("%.1f%%", percent)), 
                     vjust = -0.5, size = 3, fontface = "bold") +
  ggplot2::labs(
    title = "NA Penalized Agreement with the Actual Stock Performance by LLM Model" |> 
      stringr::str_wrap(width = 50),
    x = "Percent Agreement (NA Penalty)",
    y = "Percent of Articles with Agreement (NA Penalty) Denoted by X-Axis",
    fill = "LLM Model",
...
ggplot2::ggsave("../figs/gold_binary_agreement_na_penalized_overall.png", width = 6.5, height = 9, dpi = 600)
...
```{r gold_binary_validity_plot_na_dropped, results='hold', fig.width=6.5, fig.height=8, fig.align='center', out.width="100%", class.source = 'fold-hide', warning=FALSE, message=FALSE}
binary_gold_validity_df |>
  tidyr::drop_na(agreement_na_as_na) |>
  dplyr::group_by(chat_model, agreement_na_as_na) |>
  dplyr::summarize(count = dplyr::n()) |>
  ...
  ggplot2::geom_text(ggplot2::aes(label = sprintf("%.1f%%", percent)), 
                     vjust = -0.5, size = 3, fontface = "bold") +
  ggplot2::labs(
    title = "NA Ignored Agreement with the Actual Stock Performance by LLM Model" |> 
      stringr::str_wrap(width = 50),
    x = "Percent Agreement (NA Dropped)",
    y = "Percent of Articles with Agreement (NA Dropped) Denoted by X-Axis",
    fill = "LLM Model",
...
ggplot2::ggsave("../figs/gold_binary_agreement_na_dropped_overall.png", width = 6.5, height = 9, dpi = 600)

```

#### By Stock Performance: NA Penalized  {-}

```{r binary_validity_plot_by_perf, results='hold', fig.width=6.5, fig.height=8, fig.align='center', out.width="100%", class.source = 'fold-hide', warning=FALSE, message=FALSE}

binary_gold_validity_df |>
  dplyr::filter(!is.na(leading_news_effect)) |>
  dplyr::group_by(chat_model, leading_news_effect, agreement_na_penalty) |>
  dplyr::summarize(count = dplyr::n()) |>
  dplyr::mutate(percent = count / sum(count) * 100) |>
  dplyr::ungroup() |>
  dplyr::mutate(
  ...
  ggplot2::geom_bar(stat = 'identity', color = 'black') +
  ggplot2::scale_fill_manual(values = chat_model_colors) +
  ggplot2::facet_grid(rows = dplyr::vars(chat_model), cols = dplyr::vars(leading_news_effect)) +
  ggplot2::labs(y = "Percentage") +
  ggplot2::scale_x_continuous(breaks = seq(0,1,0.2)) +
  ggplot2::scale_y_continuous(labels = scales::percent_format(scale = 1), limits = c(0, 110), breaks = seq(0,100,25)) +
  ggplot2::geom_text(ggplot2::aes(label = sprintf("%.1f%%", percent)), 
                     vjust = -0.5, size = 3, fontface = "bold") +
  ggplot2::labs(
    title = "NA Penalized Agreement with the Actual Stock Performance",
    x = "Percent Agreement (NA Penalty)",
    y = "Percent of Articles with Agreement (NA Penalty) Denoted by X-Axis",
    fill = "LLM Model",
    caption = "Notes: The experiment was run on a sample of ~1400 news articles labeled as positive or negative by the StockNewsAPI. The text and titles of thoses articles were then passed to the LLMs for binary classification on Sept 26-29, 2024. Adding the percentages within a panel may slightly differ from 100% due to rounding errors." |> stringr::str_wrap(width = 120)
...
ggplot2::ggsave("../figs/gold_binary_agreement_na_penalized_by_sentiment.png", width = 6.5, height = 10, dpi = 600)

```


#### By By Stock Performance: NA Dropped {-}

```{r binary_validity_plot_by_perf_na_dropped, results='hold', fig.width=6.5, fig.height=8, fig.align='center', out.width="100%", class.source = 'fold-hide', warning=FALSE, message=FALSE}

binary_gold_validity_df |>
  dplyr::filter(!is.na(leading_news_effect)) |>
  tidyr::drop_na(agreement_na_as_na) |>
  dplyr::group_by(chat_model, leading_news_effect, agreement_na_as_na) |>
  dplyr::summarize(count = dplyr::n()) |>
  dplyr::mutate(percent = count / sum(count) * 100) |>
  dplyr::ungroup() |>
  dplyr::mutate(
  ...
  ggplot2::geom_text(ggplot2::aes(label = sprintf("%.1f%%", percent)), 
                     vjust = -0.5, size = 3, fontface = "bold") +
  ggplot2::labs(
    title = "NA Ignored Agreement with the Actual Stock Performance",
    x = "Percent Agreement (NA Ignored)",
    y = "Percent of Articles with Agreement (NA Ignored) Denoted by X-Axis",
    fill = "LLM Model",
    caption = "Notes: The experiment was run on a sample of ~1400 news articles labeled as positive or negative by the StockNewsAPI. The text and titles of thoses articles were then passed to the LLMs for binary classification on Sept 26-29, 2024. Adding the percentages within a panel may slightly differ from 100% due to rounding errors." |> stringr::str_wrap(width = 120)
...
ggplot2::ggsave("../figs/gold_binary_agreement_na_dropped_by_sentiment.png", width = 6.5, height = 10, dpi = 600)

```


## Ensembling the Binary Classification Labels and Comparing with the Ground Truth Labels {#ensemble_binary_classification}
...
set.seed(2024) # since we break ties by selecting randomly 

# read the binary classification ensembling results
ensembling_df = 
  binary_gold_validity_df |> 
  dplyr::filter(!is.na(leading_news_effect)) |> # remove the rows with NA ground truth)
  dplyr::select(date:rep_1, leading_news_effect) |> # as if they only ran the first replicate 
  tidyr::pivot_wider(names_from = "chat_model", values_from = "rep_1") |> 
  dplyr::rowwise() |>
  dplyr::mutate(
    agreement_na_penalty = calculate_agreement(
      reps = dplyr::c_across(`claude-3-5-sonnet-20240620`: `gpt-4o-mini`),
      ground_truth = dplyr::c_across(dplyr::contains("leading_news_effect"))
    ),
    agreement_na_as_na = calculate_agreement(
      reps = dplyr::c_across(`claude-3-5-sonnet-20240620`: `gpt-4o-mini`),
      ground_truth = dplyr::c_across(dplyr::contains("leading_news_effect")),
    ...
    ensembled_agreement = leading_news_effect == ensembled_labels_na_ignored
  ) 
...
The *mean ensemble agreement* with the ground truth labels is **`r round(mean_ensembled_agreement, digits = 3)`**. The ensemble process has been completed, and the results have been saved to a CSV file, which can be accessed [here](https://github.com/fmegahed/ai_msa/blob/main/results/binary_ensembling_df.csv).

## Prompt Sensitivity Analysis

In this section, we will conduct a prompt sensitivity analysis to assess the impact of the prompt on the binary classification labels. We will evaluate the agreement with the `gold standard` and `ground truth` labels for a simpler prompt and compare the results. The code in this section utilizes our previous code from Sections [LLM-based Labeling](#llm_labeling_bin)--[Ensembling the Binary Classification Labels and Comparing with the Ground Truth Labels](#ensemble_binary_classification). Hence, we provide little explanation prior to each code chunk within this section.


### Prompt Construction

```{python llm_binary_sensitivity, results='asis'}
# define the system prompt for the categorization task
bin_system_prompt = """
Task: You will be provided with a news article's title, full text, and a stock ticker symbol. Categorize the impact of the news article on a stock's next-day return as either "Positive" or "Negative". 
...
Note: Your classification must strictly be "Positive" or "Negative" based on the immediate expected impact. Do not include any additional analysis or explanation in your response, i.e., your response must be limited to the following two options:  
  - <classification>: Positive   
  - <classification>: Negative  
"""
...
```{python binary_labeling_sens, cache=TRUE}
res = generalized_chat_completion(
  csv_path = '../data/binary_classification_data.csv',
  columns_to_keep = ['date', 'title', 'text', 'tickers'],
  models = models,
  ...
  columns_for_chat_prompt = ['title', 'text', 'tickers'],
  num_replicates = 5,
  output_file = '../results/binary_classification_sensitivity_results.csv',
  )
```

The labeling process has been completed for the binary classification dataset. The results have been saved to a CSV file, which can be accessed [here](https://github.com/fmegahed/ai_msa/blob/main/results/binary_classification_sensitivity_results.csv).


### Validating the Binary Classification Labels

#### Extracting the LLM Labels and Creating a Dataframe for Analysis

```{r binary_classification_sens_labels, results='hold'}
# read the binary classification results from the CSV file
binary_classification_results = readr::read_csv("../results/binary_classification_sensitivity_results.csv")
...
# save the analysis dataframe as RDS and CSV files
readr::write_rds(binary_analysis_df, "../results/binary_sens_analysis_df.rds")
readr::write_csv(binary_analysis_df, "../results/binary_sens_analysis_df.csv")

```

The analysis dataframe for the binary classification experiment has been created. The dataframe contains the LLM labels for each model and replicate, along with the frequency of the most common label across replicates. The results can be accessed in the [binary_sens_analysis_df CSV file](https://github.com/fmegahed/ai_msa/blob/main/results/binary_sens_analysis_df.csv).


#### Computing the Reliability Metrics

```{r binary_sens_reliability_metrics, results='asis'}
# binary classification reliability metrics
df = reliability_coefs(binary_analysis_df, 6:10 )
...
# save the reliability metrics dataframe as RDS and CSV files
readr::write_rds(df_wide, "../results/binary_sens_reliability_metrics_means.rds")
readr::write_csv(df_wide, "../results/binary_sens_reliability_metrics_means.csv")
readr::write_rds(pa_table, "../results/binary_sens_pa_summary.rds")
readr::write_csv(pa_table, "../results/binary_sens_pa_summary.csv")
readr::write_rds(df, "../results/binary_sens_reliability_metrics.rds")
readr::write_csv(df, "../results/binary_sens_reliability_metrics.csv")
```

The CSV files containing the reliability metrics for the binary classification experiment can be accessed at [binary_sens_reliability_metrics_means.csv](https://github.com/fmegahed/ai_msa/blob/main/binary_sens_reliability_metrics_means.csv), [binary_sens_pa_summary.csv](https://github.com/fmegahed/ai_msa/blob/main/binary_sens_pa_summary.csv), and [binary_sens_reliability_metrics.csv](https://github.com/fmegahed/ai_msa/blob/main/binary_sens_reliability_metrics.csv). 


```{r binary_sens_reliability_table, results='asis', class.source = 'fold-hide', warning=FALSE}
DT::datatable(
  df_wide |> 
    dplyr::mutate(
      input_colors = dplyr::case_when(
...
Below we present a bar chart of the most frequent labels for each model across replicates.

```{r binary_sens_classification_bar_chart, results='asis', cache=TRUE, warning=FALSE, out.width='100%', fig.height=7, fig.align='center', dev=c('png', 'svg')}
# allowing for neutral as well for the sake of the data visualization
valid_words = c("Negative", "Positive", "Neutral")
...
binary_classification_results |> 
  ...
  ggplot2::geom_text(ggplot2::aes(label = sprintf("%.1f%%", percent)), 
                     vjust = -0.5, size = 3, fontface = "bold") +  # Percent on top of bars
  ggplot2::labs(
    title = "Distribution of LLM Labels for Binary Classification (Sens)",
    subtitle = paste0(
      "LLMs classified the impact of news articles on stock returns based on the *StockNewsAPI* algorithm. ",
      "The API assigned the following short-term impact labels for the articles: ", sentiment_text, ". ",
      "Each panel shows the distribution of LLM labels for a given LLM across all articles and replicates. ",
...
ggplot2::ggsave("../figs/binary_sens_classification_bar_chart.png", width = 6.5, height = 9, dpi = 600)
...
```{r binary_perc_sens_with_na_plot, results='hold', fig.width=6.5, fig.height=8, fig.align='center', out.width="100%", class.source = 'fold-hide', warning=FALSE, message=FALSE}
...
ggplot2::ggsave("../figs/binary_perc_sens_with_na_plot.png", width = 6.5, height = 8, dpi = 600)
...
validity_df = 
  readr::read_csv("../results/binary_sens_analysis_df.csv") |> 
  dplyr::select(-percent_agreement) |> 
  dplyr::left_join(stock_sentiment_df, by = c("date", "title", "text", "tickers")) |> 
  # percent of rep_1, rep_2, .., agreement with the gold standard
  dplyr::rowwise() |>
...
```{r binary_sens_validity_table, results='asis'}
validity_df |> 
  dplyr::group_by(chat_model) |>
  dplyr::summarize(
    `Mean Agreement (NA Penalty)` = mean(agreement_na_penalty) |> round(digits = 3),
...
```{r binary_sens_validity_table_by_sentiment, results='asis'}
validity_df |> 
  dplyr::group_by(chat_model, sentiment) |>
  dplyr::summarize(
    `Mean Agreement (NA Penalty)` = mean(agreement_na_penalty) |> round(digits = 3),
...
```{r binary_sens_validity_plot, results='hold', fig.width=6.5, fig.height=8, fig.align='center', out.width="100%", class.source = 'fold-hide', warning=FALSE, message=FALSE}
...
ggplot2::ggsave("../figs/sens_binary_agreement_na_penalized_overall.png", width = 6.5, height = 9, dpi = 600)
...
```{r binary_sens_validity_plot_na_dropped, results='hold', fig.width=6.5, fig.height=8, fig.align='center', out.width="100%", class.source = 'fold-hide', warning=FALSE, message=FALSE}
validity_df |>
  tidyr::drop_na(agreement_na_as_na) |>
  dplyr::group_by(chat_model, agreement_na_as_na) |>
  dplyr::summarize(count = dplyr::n()) |>
...
ggplot2::ggsave("../figs/sens_binary_agreement_na_dropped_overall.png", width = 6.5, height = 9, dpi = 600)

```

##### By Sentiment: NA Penalized  {-}

```{r binary_sens_validity_plot_by_sentiment, results='hold', fig.width=6.5, fig.height=8, fig.align='center', out.width="100%", class.source = 'fold-hide', warning=FALSE, message=FALSE}
...
ggplot2::ggsave("../figs/sens_binary_agreement_na_penalized_by_sentiment.png", width = 6.5, height = 10, dpi = 600)
...
```{r binary_sens_validity_plot_by_sentiment_na_dropped, results='hold', fig.width=6.5, fig.height=8, fig.align='center', out.width="100%", class.source = 'fold-hide', warning=FALSE, message=FALSE}
...
ggplot2::ggsave("../figs/sens_binary_agreement_na_dropped_by_sentiment.png", width = 6.5, height = 10, dpi = 600)
...
```{r sens_binary_merging_news_effects, results='asis'}
# read the stock returns dataframe
adjusted_news_df = 
  readr::read_csv("../results/binary_sens_analysis_df.csv") |> 
  dplyr::mutate(date = lubridate::as_date(date)) |> 
...
```{r sens_binary_agreement_with_ground_truth, results='asis', message=FALSE}
# calculate the agreement with the ground truth
binary_gold_validity_df = 
  binary_gold_df |>
  dplyr::rowwise() |>
...
# save the validity dataframe as RDS and CSV files
readr::write_rds(binary_gold_validity_df, "../results/binary_sens_gold_validity_df.rds")
readr::write_csv(binary_gold_validity_df, "../results/binary_sens_gold_validity_df.csv")
...
```{r binary_sens_gold_validity_table, results='asis'}
binary_gold_validity_df |> 
  dplyr::group_by(chat_model) |>
  dplyr::summarize(
    `Mean Agreement (NA Penalty)` = mean(agreement_na_penalty) |> round(digits = 3),
...
```{r binary_sens_validity_table_by_news, results='asis'}
binary_gold_validity_df |> 
  dplyr::group_by(chat_model, leading_news_effect) |>
  dplyr::summarize(
    `Mean Agreement (NA Penalty)` = mean(agreement_na_penalty) |> round(digits = 3),
...
```{r gold_binary_sens_validity_plot, results='hold', fig.width=6.5, fig.height=8, fig.align='center', out.width="100%", class.source = 'fold-hide', warning=FALSE, message=FALSE}
...
ggplot2::ggsave("../figs/sens_gold_binary_agreement_na_penalized_overall.png", width = 6.5, height = 9, dpi = 600)
...
```{r gold_binary_sens_validity_plot_na_dropped, results='hold', fig.width=6.5, fig.height=8, fig.align='center', out.width="100%", class.source = 'fold-hide', warning=FALSE, message=FALSE}
binary_gold_validity_df |>
  tidyr::drop_na(agreement_na_as_na) |>
  dplyr::group_by(chat_model, agreement_na_as_na) |>
  dplyr::summarize(count = dplyr::n()) |>
...
ggplot2::ggsave("../figs/sens_gold_binary_agreement_na_dropped_overall.png", width = 6.5, height = 9, dpi = 600)

```

##### By Stock Performance: NA Penalized  {-}

```{r sens_binary_validity_plot_by_perf, results='hold', fig.width=6.5, fig.height=8, fig.align='center', out.width="100%", class.source = 'fold-hide', warning=FALSE, message=FALSE}
...
ggplot2::ggsave("../figs/sens_gold_binary_agreement_na_penalized_by_sentiment.png", width = 6.5, height = 10, dpi = 600)
...
```{r sens_binary_validity_plot_by_perf_na_dropped, results='hold', fig.width=6.5, fig.height=8, fig.align='center', out.width="100%", class.source = 'fold-hide', warning=FALSE, message=FALSE}
...
ggplot2::ggsave("../figs/sens_gold_binary_agreement_na_dropped_by_sentiment.png", width = 6.5, height = 10, dpi = 600)

```


#### Ensembling the Binary Classification Labels and Comparing with the Ground Truth Labels

Many practitioners may run their experiment only once and may not have the luxury of running multiple replicates to assess the reliability of the LLM labels. In such cases, ensembling the labels from multiple replicates may provide a more reliable estimate of the LLM's performance. In this section, we will ensemble the binary classification labels from the replicates and assess the validity of the ensembled labels.

```{r sens_binary_classification_ensembling, results='asis', message=FALSE}
set.seed(2024) # since we break ties by selecting randomly 
...
# save the ensembling dataframe as RDS and CSV files
readr::write_rds(ensembling_df, "../results/sens_binary_ensembling_df.rds")
readr::write_csv(ensembling_df, "../results/sens_binary_ensembling_df.csv")
```

The *mean ensemble agreement* with the ground truth labels is **`r round(mean_ensembled_agreement, digits = 3)`**. The ensemble process has been completed, and the results have been saved to a CSV file, which can be accessed [here](https://github.com/fmegahed/ai_msa/blob/main/results/sens_binary_ensembling_df.csv).
