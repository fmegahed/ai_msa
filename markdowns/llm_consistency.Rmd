...
```{r binary_classification_labels, results='hide'}
...
# function to calculate the most frequent value (Mode) and its frequency
...
binary_analysis_df = 
  ...
  dplyr::mutate(
    # extract the LLM labels, handling both patterns with an optional colon
    llm_label = stringr::str_extract(
      chat_response, 
      "(?i)(<classification>:?\\s?\\w+|classification:?\\s?\\w+)"
    ),
    # remove the "classification" part
    llm_label = stringr::str_replace(
      llm_label, "(?i)(<*classification>:?\\s?|classification: )", ""
      ),
    # convert the LLM labels to lowercase for consistency
    llm_label = stringr::str_to_lower(llm_label)
  ) |>
